    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.mensage === 'full') {
                $('.buttons_event_' + id).each(function(i, el) {
                    $(el).html(button_event(id, '#a0a0a0', 'LOTADO', 'send_event_enter'));
                });
                $('.inscritos_event_' + id).each(function(i, el) {
                    $(el).html(result.subscribers);
                });
                toastr.error("<span>" + result.infos + "</span>", "OPSS!");
            } else if (result.mensage === 'conflict') {
                $('.buttons_event_' + id).each(function(i, el) {
                    $(el).html(button_event(id, '#1AB394', 'ENTRAR NA PROGRAMAÇÃO', 'send_event_enter'));
                });
                ...
                $('.buttons_event_' + id).each(function(i, el) {
                    $(el).html(button_event(id, '#ef1717', 'SAIR DA PROGRAMAÇÃO', 'send_event_out'));
                });
...
jQuery(document).ready(function($) {
  ...
  $('.filtro').on('ifUnchecked', function(event){
     $('.'+event.target.name).each(function(i, el) {
        $(el).fadeOut();
      });
      $('.li_'+event.target.name).removeClass('active');
  });

  $('.filtro').on('ifChecked', function(event){
     $('.'+event.target.name).each(function(i, el) {
        $(el).fadeIn();
      });
     $('.li_'+event.target.name).addClass('active');
  });
